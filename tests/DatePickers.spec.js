...
test('Datepickers', async ({page})=>{
    ...
    const DatePickerPage = await page.getByRole('link',{name:'Date Pickers'})
    await DatePickerPage.click()
    const EnterDate = await page.locator('#single-input-field')
    await EnterDate.click()

    await page.locator('.datepicker-days th.datepicker-switch').click() //locator chaining -- .parent_class space child_tagname.child_class name 
    ...
    const startYear = await yearPicker.split('-')[0]   //string spliter with ' and saving 1st index value [0]
    const endYear = await yearPicker.split('-')[1]
